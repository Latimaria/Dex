...
JNIEXPORT jint JNICALL Agent_OnAttach(JavaVM* vm, char* options, void* reserved){
    ...
    for (int i = 0; i < class_count; i++) {
        ...
        if (strcmp(signature, "Lcom/example/timing/logX;") == 0) {
...
void JNICALL ClassFileLoadHook(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jclass class_being_redefined, jobject loader, const char* name, jobject protection_domain,
    ...
    if(strcmp(name, "com/example/timing/logX")==0){
    ...
    jstring file_path = jni_env->NewStringUTF("/data/data/com.example.timing/files/classes_CG.dex");
